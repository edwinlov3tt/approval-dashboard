...
import type { ApprovalActivity } from '@/types/approval';
      ...
      {activities.map((activity, index) => {
        ...
        return (
          <div key={activity.id} className="relative flex gap-4">
            ...
            {/* Icon */}
            <div className={`relative flex-shrink-0 w-10 h-10 rounded-full ${colorClass} flex items-center justify-center`}>
                  ...
                  {(activity.user_name || activity.user_email) && (
                    <div className="flex items-start gap-1.5 mt-1 text-xs text-gray-600">
                      ...
                      <div className="flex flex-col">
                        ...
                        {activity.user_email && <span className="text-gray-500">{activity.user_email}</span>}
                      </div>
                    </div>
                  )}
                  ...
                  {activity.metadata && (
                    <div className="mt-2 text-xs text-gray-600">
                      {activity.metadata.comments && (
                        <p className="bg-gray-50 rounded-md p-2 border border-gray-200">
                          {activity.metadata.comments}
                        </p>
                      )}
                      ...
                      {activity.metadata.revision_count !== undefined && activity.metadata.revision_count > 0 && (
                        <div className="bg-orange-50 rounded-md p-2 border border-orange-200 mt-2">
                      ...
                      {activity.metadata.geo && (
                        <div className="bg-blue-50 rounded-md p-2 border border-blue-200 mt-2 flex items-center gap-2">
                          ...
                          <div className="text-blue-800">
                            {activity.metadata.geo.city && activity.metadata.geo.region && (
                              <span>{activity.metadata.geo.city}, {activity.metadata.geo.region}</span>
                            )}

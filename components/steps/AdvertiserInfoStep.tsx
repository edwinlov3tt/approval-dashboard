...
export const AdvertiserInfoStep: React.FC<AdvertiserInfoStepProps> = ({ fullPage = false, isPreview = false }) => {
        ...
        try {
          ...
          for (const [filename, zipEntry] of Object.entries(contents.files)) {
            if (!zipEntry.dir && /\.(png|jpe?g|webp)$/i.test(filename)) {
              ...
              await processCreativeUpload(imageFile);
            }
          }
  ...
  if (fullPage) {
    return (
      <div className="w-full">
        <div className="mb-4">
          <h2 className="text-2xl font-semibold text-text-primary mb-2">Step 1: Advertiser Info</h2>
          <p className="text-14 text-text-muted">Provide brand context, website, and goals to unlock the next step.</p>
        </div>
